...
Component({
  ...
  methods: {
    ...
    initGameApplication(canvas, options) {
      ...
      const contextText = application.renderer instanceof PIXI.CanvasRenderer ? "pixi" : "webGl";
    ...
    onTouchHandle(event) {
      if (this.mainCanvas && event.changedTouches && event.changedTouches.length) {
        ...
        if (this.props.onTouchHandle) {
          this.props.onTouchHandle({
            type: event.type,
            touches: event.touches.map((touchItem) => {
              const x = touchItem.clientX;
              const y = touchItem.clientY;
              return { x, y };
            })
          });
        }
